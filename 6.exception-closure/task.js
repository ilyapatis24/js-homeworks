...
function parseCount(products) {
  let resultParse = parseFloat(products);
...
class Triangle {
  ...
  get area() {
    let square = this.perimeter / 2;
    return +(Math.sqrt(square * (square - this.sideOne) * (square - this.sideTwo) * (square - this.sideThree))).toFixed(3);
  }
}
